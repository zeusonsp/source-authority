...
- **Resend domain verification pending** (rastreado desde 2026-05-07, B3-followup): `sourceauthority.com.br` foi adicionado ao Resend com 3 records DNS criados na Cloudflare (DKIM TXT, SPF MX, SPF TXT). DKIM verifica intermitentemente (oscila); SPF/MX `pending` há 30+ min apesar das records resolverem corretamente em todos os resolvers públicos (8.8.8.8, 1.1.1.1, 9.9.9.9, OpenDNS, e ambos NS Cloudflare ken/nadia). Provável caching interno do Resend. **Estado atual de produção:** `from: onboarding@resend.dev` (sandbox Resend), `LEADS_NOTIFICATION_EMAIL: zeusonsp@gmail.com`. Quando Resend mostrar `verified`, basta: (1) editar `apps/landing/src/app/demo/actions.ts:notifyLead()` linha do `from:` pra `noreply@sourceauthority.com.br` e (2) `vercel env rm LEADS_NOTIFICATION_EMAIL production && vercel env add ... nathanturra_r@hotmail.com production && vercel --prod`. Total: 2 commits + 1 deploy.
