...
- **Resend `LEADS_NOTIFICATION_EMAIL` apontando pra `zeusonsp@gmail.com`** (rastreado desde 2026-05-07, B3): limitação do Resend free tier sem domain verification — só permite enviar pro e-mail da conta (`zeusonsp@gmail.com`). B2 envia notificação de lead pra esse e-mail. Resolução: B3 verifica `sourceauthority.com.br` no Resend (SPF/DKIM via Cloudflare DNS, após migração do Registro.br), e troca destino pra `nathanturra_r@hotmail.com`. Mudança = 1 linha no `.env.local` e ajustar TODO em `apps/landing/src/app/demo/actions.ts:notifyLead()`.
